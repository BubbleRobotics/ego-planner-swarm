...
namespace ego_planner
{
  ...
  // Return multiple safe sets of control points
  std::vector<ControlPoints> BsplineOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments)
  {
    ...
    constexpr int MAX_TRAJS = 8;                                                                            // Maximum number of trajectories
    constexpr int VARIS = 2;                                                                                // Number of variation types allowed
    int seg_upbound = std::min((int)segments.size(), static_cast<int>(floor(log(MAX_TRAJS) / log(VARIS)))); // Upper bound on number of segments to vary
    std::vector<ControlPoints> control_pts_buf;
    control_pts_buf.reserve(MAX_TRAJS);
    const double RESOLUTION = grid_map_->getResolution();
    const double CTRL_PT_DIST = (cps_.points.col(0) - cps_.points.col(cps_.size - 1)).norm() / (cps_.size - 1); // Average distance between control points
    ...
    for (int i = 0; i < seg_upbound; i++)
    {
      ...
      if (RichInfoSegs[i].first.size > 1)
      {
        ...
        {
          // Iterate every control point and its next one, sample by linear interpolation
          ...
          for (double a = 1; a > 0; a -= step_size)
          {
            ...
            // If inside an inflated obstacle, record index and position
      ...
      exit_multi_loop1:;
        // Find the last intersection with an obstacle in this segment
      ...
      exit_multi_loop2:;
        ...
        // double check
        // If the start or end control point of a segment is in obstacle, this segment will be removed
        ...
        for (int j = occ_start_id; j <= occ_end_id; j++)
        {
          ...
          // Check that each control point has exactly one base point
          if (RichInfoSegs[i].first.base_point[j].size() != 1)
          {
            ...
            RCLCPP_ERROR(rclcpp::get_logger("distinctiveTrajs"), "Wrong number of base_points!!! Should not happen!.");
          ...
          // For the starting control point in this segment, use occ_start_pt as reverse base point
          if (j == occ_start_id)
          {
            base_pt_reverse = occ_start_pt;
          }
          // For the ending control point, use occ_end_pt as reverse base point
          else if (j == occ_end_id)
          {
            base_pt_reverse = occ_end_pt;
          }
          // For middle control points, shift along opposite direction with same distance as original base point
          else
          {
          ...
          // Check if base_pt_reverse is inside an obstacle
          if (grid_map_->getInflateOccupancy(base_pt_reverse)) // Search outward.
          {
            ...
            {
              // Move the base point outwards until it is outside obstacles
            ...
            // If not found, drop this segment
            if (l > l_upbound)
            {
          ...
          // If far enough from control point and not inside obstacle, just keep it
          ...
          // If base point and control point are too close, drop this segment
          else
          {
        ...
        {
          // For control points before the first collision, use the same base point and direction as at occ_start_id
          ...
          // For control points after the last collision, use the same base point and direction as at occ_end_id
      ...
      // Case when the segment has only one control point
    ...
    // Combine each segment to form new control point sequences
    if (seg_upbound == 0) // After the erase operation above, segment length will decrease to 0 again.
    {
      std::vector<ControlPoints> oneSeg;
      oneSeg.push_back(cps_);
      return oneSeg;
    ...
    for (int i = 0; i < max_traj_nums; i++)
    {
      ...
      int digit_id = 0;
      selection[digit_id]++;
      // Generate a selection table in base VARIS
      ...
      {
        // If the current control point is not inside the current segment, copy from original cps_
        ...
        // If cp_id is within the current segment, choose first or second version based on selection[seg_id]
        else if (cp_id >= segments[seg_id].first && cp_id <= segments[seg_id].second)
        {
          if (!selection[seg_id]) // choose the first variant
          {
          ...
          else // choose the second variant
          {
          ...
          // If we reached the last control point of this segment, reset cp_of_seg_id and go to next segment
  ...
  // Initialize control points for collision-based segmentation
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    // Required stable interval length when entering or leaving obstacles
    constexpr int ENOUGH_INTERVAL = 2;
    // Sampling step for obstacle checking
    ...
    // Flags for whether we have found start and end of a segment
    ...
    for (int i = order_; i <= i_end; ++i)
    {
      ...
      for (double a = 1.0; a > 0.0; a -= step_size)
      {
        ...
        if (occ && !last_occ)
        {
          ...
          // If we enter obstacle, invalidate a pending exit
        ...
        // If we have left the obstacle and the state is stable enough, mark as valid end
        ...
        // When both start and end found, record the segment and reset flags
    ...
    // If no segments inside obstacles, return empty
    ...
    /*** a star search ***/
    // For each obstacle segment, search a path between its start and end using A*
    ...
    // Iterate each segment and compute its allowed bounds
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      if (i == 0) // first segment
      {
        ...
        {
          // Upper bound is middle index between current segment end and next segment start
          id_up_bound = (int)(((segment_ids[0].second + segment_ids[1].first) - 1.0f) / 2); // id_up_bound : -1.0f fix()
        }
        else
        {
          // For last segment, leave order_+1 points at the end as free
      ...
      {
        // Lower bound is middle index between previous segment end and this segment start
        id_low_bound = (int)(((segment_ids[i].first + segment_ids[i - 1].second) + 1.0f) / 2); // id_low_bound : +1.0f ceil()
        // Upper bound is near the end, leaving order_+1 points
        id_up_bound = init_points.cols() - order_ - 1;
      }
      else
      {
        // Lower bound: middle index between previous segment end and this segment start
        id_low_bound = (int)(((segment_ids[i].first + segment_ids[i - 1].second) + 1.0f) / 2); // id_low_bound : +1.0f ceil()
        // Upper bound: middle index between this segment end and next segment start
    ...
    // Avoid overlap between adjusted segments
    ...
    {
      // step 1
      // For this segment, mark all temporary flags as false
      ...
      {
        // Direction of control points
        Eigen::Vector3d ctrl_pts_law(init_points.col(j + 1) - init_points.col(j - 1)), intersection_point;
        // Take the middle A* path point
        int Astar_id = a_star_pathes[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
        // Dot product to check which side of ctrl_pts_law the A* point lies
        ...
        while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
        {
          ...
          // Move along the path based on sign of val
          if (val >= 0)
            --Astar_id;
          else
            ++Astar_id;
          ...
          // When crossing the line, compute intersection point between A* path and direction from the control point
        ...
        {
          // Distance from control point to intersection
          double length = (intersection_point - init_points.col(j)).norm();
          if (length > 1e-5)
          {
            cps_.flag_temp[j] = true;
            // Sample along the line from control point to intersection
            for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
            {
              ...
              if (occ || a < grid_map_->getResolution())
              {
                ...
                // Record base point and direction from control point
      ...
      // Corner case when segment has only two control points, trajectory is too short
      if (segment_ids[i].second - segment_ids[i].first == 1)
      {
        // Direction and middle point of the two control points
        ...
        // Same logic but with middle point instead of control point
        int Astar_id = a_star_pathes[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
        ...
        while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
        {
          ...
          if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
          {
            ...
            // If distance is big enough, store corresponding information
      ...
      if (got_intersection_id >= 0)
      {
        // For control points after the intersection, copy previous valid base point and direction
        ...
        // For control points before the intersection, copy from the next control point
      ...
      {
        // Just ignore; this segment has no valid direction
  ...
  // Early exit callback for emergency stop
  ...
  // Use combineCostRebound to compute cost
  ...
  // Use combineCostRefine to compute refined cost
  ...
  // Several cost computation functions
                                                 ...
                                                 Eigen::MatrixXd &gradient, int iter_num, double smoothness_cost)
  {
    ...
    // If trajectory is smooth enough after some iterations, run collision check and rebound
    if (iter_num > 3 && smoothness_cost / (cps_.size - 2 * order_) < 0.1) // 0.1 indicates sufficient smoothness
    {
                                             ...
                                             Eigen::MatrixXd &gradient)
  {
    ...
    for (int i = 0; i < q.cols() - 1; i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        if (vi(j) > max_vel_ + demarcation)
        {
          ...
          cost += (ar * diff * diff + br * diff + cr) * ts_inv3; // multiply ts_inv3 to make vel and acc have similar magnitude
    ...
    for (int i = 0; i < q.cols() - 1; i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        if (vi(j) > max_vel_)
        {
          // Outside positive velocity bound
          cost += pow(vi(j) - max_vel_, 2) * ts_inv2; // multiply ts_inv2 to make vel and acc have similar magnitude
        ...
        {
          // Outside negative velocity bound
    ...
    for (int i = 0; i < q.cols() - 2; i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        if (ai(j) > max_acc_)
        {
          // Outside positive acceleration bound
        ...
        {
          // Outside negative acceleration bound
          cost += pow(ai(j) + max_acc_, 2);
  ...
  /*                    The seven functions above all compute costs           */

  // Check whether there is collision and generate rebound directions
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        {
          cout.precision(2);
          // If current point is outside all stored collision base points, treat it as non-collision
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        // step 1: clear temporary flags
        for (int j = segment_ids[i].first; j <= segment_ids[i].second; ++j)
          cps_.flag_temp[j] = false;

        // step 2: compute base points and directions based on A* path
        int got_intersection_id = -1;
        for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
        {
        ...
        // step 3: propagate base point / direction along the segment
        if (got_intersection_id >= 0)
        {
  ...
  // Set time interval ts and call rebound_optimize(final_cost) to push trajectory out of obstacles;
  // return optimal collision-free trajectory in optimal_points
  ...
  // Given initial control points control_points and time interval ts, call rebound_optimize(final_cost)
  // to push the trajectory out of obstacles, and return optimal collision-free trajectory
  ...
  // Given initial control points init_points and time interval ts, call refine_optimize() to re-allocate time,
  // obtain a dynamically feasible trajectory and return control points in optimal_points
  bool BsplineOptimizer::BsplineOptimizeTrajRefine(const Eigen::MatrixXd &init_points, const double ts, Eigen::MatrixXd &optimal_points)
  {
    ...
    // Store control points and time interval
  ...
  // Optimize objective using L-BFGS and get smooth, collision-free, dynamically feasible, swarm-safe trajectory with terminal cost
  bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    do
    {
      ...
      // Initialize control variable array from cps_.points
      ...
      lbfgs_params.mem_size = 16; // Number of historical steps stored by the algorithm
      ...
      // If converged, reached max iterations, already minimized, or stopped, run collision check phase
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        /*** collision check, phase 1 (swarm distance) ***/
        if ((min_ellip_dist_ != INIT_min_ellip_dist_) && (min_ellip_dist_ > swarm_clearance_))
        {
          // Too close to swarm, restart optimization with increased weight
          success = false;
          restart_nums++;
          initControlPoints(cps_.points, false);
          new_lambda2_ *= 2; // Increase avoidance weight
        ...
        /*** collision check, phase 2 (obstacle collision along trajectory) ***/
        // Create a uniform B-spline trajectory
        UniformBspline traj = UniformBspline(cps_.points, 3, bspline_interval_);
        double tm, tmp;
        traj.getTimeSpan(tm, tmp);
        // Step size chosen so each step is about one grid cell
        double t_step = (tmp - tm) / ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() / grid_map_->getResolution());
        // Only check front 2/3 of the trajectory for obstacle collision
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          flag_occ = grid_map_->getInflateOccupancy(traj.evaluateDeBoorT(t));
          if (flag_occ)
          {
            // If collision happens very early, first three control points are in obstacle -> infeasible
        ...
        /*** collision check, phase 3 (optional clearance check) ***/
// #define USE_SECOND_CLEARENCE_CHECK
        ...
        // If no collision after all checks, declare success
        if (!flag_occ)
        {
          printf("\033[32miter(+1)=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost=%5.3f\n\033[0m", iter_num_, time_ms, total_time_ms, final_cost);
          success = true;
        }
        // If collision remains, restart with re-initialized control points
        else // restart
        {
          restart_nums++;
          initControlPoints(cps_.points, false);
      ...
      // Optimization canceled by earlyExit
  ...
  // Optimize objective using L-BFGS and get smooth, well-fitted and dynamically feasible trajectory with refined timing
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      // Use optimized control points to create trajectory and check for collision again
      UniformBspline traj = UniformBspline(cps_.points, 3, bspline_interval_);
      double tm, tmp;
      traj.getTimeSpan(tm, tmp);
      // Step size equals approximately one grid cell
      double t_step = (tmp - tm) / ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() / grid_map_->getResolution()); // Step size is defined as the maximum size that can pass through every grid.
      for (double t = tm; t < tmp * 2 / 3; t += t_step)
      {
        if (grid_map_->getInflateOccupancy(traj.evaluateDeBoorT(t)))
        {
          // Refined trajectory hits obstacle
          // Convert ref_pts_ to matrix form (for potential debugging)
      ...
      // If collision exists, increase lambda4_ and try again
      if (!flag_safe)
        lambda4_ *= 2;
  ...
  // Combine and compute rebound cost and gradient
  ...
  // Combine and compute refined cost and gradient
